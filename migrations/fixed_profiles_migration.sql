-- Fixed Migration: Add tool_token_balance and is_member columns (no OLD references)
-- Created: 2026-04-17
...
-- Step 5: Create RLS policies (simplified without OLD references)
DROP POLICY IF EXISTS "Users can view own profile" ON profiles;
CREATE POLICY "Users can view own profile" ON profiles FOR SELECT USING (auth.uid() = id);

DROP POLICY IF EXISTS "Users can update own profile info" ON profiles;
CREATE POLICY "Users can update own profile info" ON profiles FOR UPDATE USING (auth.uid() = id);
